...
import { useLocalUser, MovieLibraryStatus } from '../context/UserContext';
...
const MovieStatusButton: React.FC<MovieStatusButtonProps> = ({ movie, className = '', minimal = false }) => {
    ...
    const formatStatus = (status: MovieLibraryStatus) => {
    ...
    const handleStatusChange = (status: MovieLibraryStatus) => {
        updateMovieStatus(movie, status);
        setIsOpen(false);
    };

    const handleRemove = () => {
        removeFromMovieLibrary(movie.id);
        setIsOpen(false);
    };

    const statuses: MovieLibraryStatus[] = ['watched', 'plan_to_watch', 'on_hold', 'dropped'];
                ...
                {isOpen && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-miru-surface/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl py-2 z-50 animate-fade-in-up origin-top">
                        {statuses.map(status => (
                            ...
                            >
                                {status === 'watched' && <span className="w-2 h-2 rounded-full bg-blue-500"></span>}
    ...
    return (
        <div className={`relative ${className}`} ref={dropdownRef}>
            <button
                onClick={toggleDropdown}
                className={`w-full ${minimal ? 'py-2 px-3 text-xs' : 'py-3.5 px-6'} rounded-xl bg-miru-surface border border-miru-primary/50 text-miru-primary font-bold hover:bg-miru-primary/10 transition-all flex items-center justify-center gap-2`}
            >
